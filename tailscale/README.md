...
- Cardinality: one asset per device. A device with several Tailscale addresses and several physical endpoints is still one record with several interfaces.
...
Up to two interfaces are built, and they are different kinds of thing:
...
- **The physical interface**, from `clientConnectivity.endpoints` — the public and private addresses the client last advertised for direct connections. **These are the ones that can match a scanned asset**, because a private endpoint such as `192.168.1.20:41641` is the device's real LAN address. `network_interface()` strips the `addr:port` and `[ipv6]:port` forms automatically.

That split has a direct consequence: a device whose `clientConnectivity` is absent — which is normal for a node that has not recently connected, and for any response where the field is not populated — arrives with only overlay addresses and effectively cannot merge onto a scanned asset by IP at all. **No MAC address is available from this API**, so hostname is the only remaining signal.
...
* The integration selects its authentication mode from `client_id`: supply it and the script exchanges the client credentials for an access token, leave it blank and `api_key_or_client_secret` is used directly as a bearer token.
