...
- Uniqueness scope: the organization named by `organization_key`, which appears in the path of every request. The composed runZero id carries the Org Key as its scope, so two Carbon Black organizations imported through one custom integration keep separate id spaces.
...
- Final runZero ID: `carbon-black:<org_key>:<device id>`, e.g. `carbon-black:A1B2C3D4:4839201`. The leaf is the raw Carbon Black device id; the scope is the Org Key Carbon Black itself assigns to the organization and puts in every request path.
...
Earlier revisions emitted the bare device id (`4839201`) as the foreign id. The id is now scoped by the Org Key (`carbon-black:<org_key>:<device id>`), so existing foreign records are re-identified once: the next run creates records under the new ids, which merge back onto the same runZero assets via the MAC, IP, and hostname correlators, and the old foreign records age out. The new id is stable across renames, address changes, and policy moves, and keeps two Carbon Black organizations from colliding in one runZero organization.
