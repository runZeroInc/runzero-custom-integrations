...
CONFIG = {
    ...
    "params": [
        ...
        {
            # dst_url and rz_account_token are declared optional because this
            # field can supply either of them instead. main() resolves each from
            # the individual param first and this JSON second, and refuses the
            # run with a clear message when neither carries it -- the check has
            # to live there rather than in an atLeastOneOf, which cannot be
            # satisfied by the validator when it spans optional params.
...
def main(*args, **kwargs):
    ...
    # The legacy blob is consulted BEFORE the src_url param, which is the
    # reverse of the fields below, because src_url is the only one carrying a
    # CONFIG default. Defaults are applied before the script runs, so
    # kwargs['src_url'] is always populated and would always beat a legacy
    # value -- pointing a legacy-configured run at console.runzero.com instead
    # of the console it names. The other fields have no default, so an absent
    # param is genuinely absent there and the param may win.
    src_url = creds.get('src_url') or kwargs.get('src_url') or 'https://console.runzero.com'
