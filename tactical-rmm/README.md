...
- **That one request gets one attempt.** Streaming needs the raw body, so the request uses the `http.get` builtin, and `retries` is a parameter of `get_json`/`post_json` only — a raw request has no retry budget and cannot opt into one. A transient failure on the agent list ends the run with a logged error rather than importing a partial estate. The per-agent software requests do use `get_json` and so do get the default three retries, including `Retry-After` handling on a 429.
