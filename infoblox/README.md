...
- Optional joins, keyed on address within a single network view:
  - `lease` (**Import DHCP leases**, on by default) contributes the MAC when IPAM has none, the DHCP client hostname, binding state, serving member, DHCP option 61 uid, and the fingerprint. `starts` becomes `firstSeenTS`, and `cltt` (falling back to `ends`) becomes `lastSeenTS`. These are the only lifecycle timestamps available — the `ipv4address` object has none. The parsed values are clamped to now: an active lease's `ends` is in the future by definition, and the platform silently drops any record whose seen timestamps are ahead of now. The raw epoch values survive in the `infoblox_dhcp_*` attributes.
