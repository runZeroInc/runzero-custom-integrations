...
- Uniqueness scope: the Automox organization. The composed runZero id carries the organization id as its scope — the row's own `organization_id` when `/api/servers` reports one, else the organization resolved via `/api/orgs` — so two Automox organizations imported through one custom integration keep separate id spaces. A row with neither (no `organization_id` and no readable `/api/orgs`) is skipped with a log line rather than emitted under an id that would collide or later change.
...
- Final runZero ID: `automox:<organization id>:<device id>`, e.g. `automox:1234:4821993`. The leaf is the raw Automox device id; the scope is the organization id Automox itself assigns, taken from the device row's `organization_id` (preferred, so the id never depends on the `organization_hint` configuration) with the `/api/orgs`-resolved organization as the fallback.
...
Earlier revisions emitted the bare device id (`4821993`) as the foreign id. The id is now scoped by the Automox organization id (`automox:<organization id>:<device id>`), so existing foreign records are re-identified once: the next run creates records under the new ids, which merge back onto the same runZero assets via the MAC, IP, and hostname correlators, and the old foreign records age out. The new id is stable across renames, address changes, and `organization_hint` edits, and keeps two Automox organizations from colliding in one runZero organization.
