...
- Labels become runZero tags in `key:value` form (`role:Web`, `env:Production`, `app:HRM`, `loc:US`). A workload's `labels[]` may arrive either expanded or as bare `href` references depending on the PCE release, and the expanded representation is not portable across releases, so the organization's label catalog is fetched once from `GET /api/v2/orgs/{org_id}/labels` and used to resolve references. An organization with more than 500 labels will hit the collection cap on that call; the shortfall is logged and the affected labels import as references only.
...
- The asynchronous result is one downloadable file, so the raw records unavoidably arrive together; each asset is built and reported in turn, so only one `ImportAsset` object exists at a time.
