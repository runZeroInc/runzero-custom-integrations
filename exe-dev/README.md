...
- **There are no fixtures for this integration.** Every other integration in this repository can be exercised offline against recorded API shapes; here the command-line run against a real token is the only test. That is worth changing before the parsing is extended further, because a response-shape change on any of the four commands currently fails at runtime rather than in a test.
