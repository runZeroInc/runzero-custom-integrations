...
- **Page the list calls.** `/api/v1/nodes`, `/api/v1/services`, and `/api/v1/pods` are each fetched in a single request with no `limit` or `continue` handling, so the entire collection arrives in one response and is held in memory before any asset is built. Kubernetes supports chunked list requests precisely for this; a large cluster is where the current shape will hurt first, and the pod list is where it will hurt first of the three. The `resourceVersion` returned alongside is also the basis for an incremental sync rather than a full walk.
