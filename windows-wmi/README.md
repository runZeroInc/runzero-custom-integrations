...
2. [Create the Credential for the Custom Integration](https://console.runzero.com/credentials).
   ...
   - `wmi_namespace` is written in the DCE/RPC spelling (`//./root/cimv2`). On a WinRM transport the script strips the leading `/` and `.` itself, so one value works for all four transports — do not enter two different spellings.
