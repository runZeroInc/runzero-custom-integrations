...
def get_token(client_id, client_secret, base_url, scope=None):
    ...
    response = http_post(token_url, headers=headers, params=params)
...
def start_nql_export(token, api_url, query_id):
...
def get_export_status(token, api_url, export_id):
...
def fetch_all_export_rows(token, api_url, query_id, client_id, client_secret, auth_url, scope):
    export_id = start_nql_export(token, api_url, query_id)
    if export_id == "__UNAUTHORIZED__":
        token = get_token(client_id, client_secret, auth_url, scope=scope)
        if not token:
            return []
        export_id = start_nql_export(token, api_url, query_id)
    if not export_id:
    ...
    for _ in range(0, 120):
        status_data = get_export_status(token, api_url, export_id)
        if not status_data:
            return []
    ...
    download_response = http_get(results_file_url)

    if download_response.status_code == 406:
        download_response = http_get(results_file_url, headers={"Accept": "text/csv"})
...
def main(**kwargs):
    ...
    if not client_id or not client_secret:
        print("Missing credentials! Please configure access_key and access_secret in runZero.")
        return []
    ...
    rows = fetch_all_export_rows(token, API_URL, QUERY_ID, client_id, client_secret, AUTH_URL, SCOPE)
