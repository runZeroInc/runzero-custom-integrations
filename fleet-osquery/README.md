...
- Fleet is self-hosted, so servers of many ages are in the field. If a server rejects the request as invalid (400/404/422), the optional `populate_*` and `additional_info_filters` parameters are dropped once, the page is retried, and the run continues with basic inventory. A 401/403 or a transient status is reported instead, never silently downgraded.
