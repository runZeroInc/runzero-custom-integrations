...
def main(*args, **kwargs):
    ...
    if err or status != "200":
        print("get_tasks failed:", err if err else "status {}".format(status))
        gmp_close(state)
        return None
    ...
    for task in tasks:
        ...
        if cutoff != None and scan_end:
            ...
            if ended != None and ended.unix < cutoff.unix:
                print("greenbone: skipping task '{}' (last report {} older than {} days)".format(
                    task_name, scan_end, max_age_days))
                continue
